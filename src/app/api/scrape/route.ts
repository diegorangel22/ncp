 ...
 * - NEXT_PUBLIC_BASE_PATH (e.g. "/ncp")
 * - WORKER_MAX (default max sections requested from Worker)
 * - MAX_SECTIONS (hard cap after Worker returns)
 * - UPLOAD_CONCURRENCY (default 2)
 * - ABSOLUTE_IMAGE_URLS ("1" => return absolute URLs; default = relative)
 * - PUBLIC_SITE_ORIGIN (optional override for absolute URLs)
 */
...
function safeDataUrlToBytes(maybe: unknown): Uint8Array | null {
  if (typeof maybe !== "string") return null;
  const i = maybe.indexOf(","); if (i < 0) return null;
  const b64 = maybe.slice(i + 1);
  try { const buf = Buffer.from(b64, "base64"); return new Uint8Array(buf.buffer, buf.byteOffset, buf.byteLength); }
  catch { return null; }
}
function safeBase64FromDataUrl(maybe: unknown): string | null {
  if (typeof maybe !== "string") return null;
  const i = maybe.indexOf(","); return i >= 0 ? maybe.slice(i + 1) : null;
}
async function pMap<T,R>(items: T[], limit: number, worker: (item:T, idx:number)=>Promise<R>): Promise<R[]> {
...
export async function POST(req: Request) {
  ...
  if (!h || !ORIGINS.includes(h)) {
    return new Response(JSON.stringify({ error: "Origin not allowed" }), {
      status: 403, headers: { "content-type": "application/json", ...cors(h) },
    });
  }
  type Body = { domain?: string };
  let domain: string | undefined;
  try { const body = (await req.json()) as Body; domain = body.domain; } catch { domain = undefined; }
  if (!domain) {
    return new Response(JSON.stringify({ error: "Missing domain" }), {
      status: 400, headers: { "content-type": "application/json", ...cors(h) },
    });
  }
...
async function handle(origin: string, req: Request, h: string | null) {
  try {
    ...
    const raw = u.searchParams.get("raw") === "1";        // pass-through mode
    const includeB64 = u.searchParams.get("b64") === "1"; // passthrough of *_b64 if present

    // Worker max
    const qMax = Number(u.searchParams.get("max") || "");
    const workerMax = clamp(qMax || DEFAULT_WORKER_MAX, 1, 12);
    const maxSections = clamp(
      Number(process.env.MAX_SECTIONS || DEFAULT_MAX_SECTIONS || workerMax), 1, workerMax
    );

    // Build Worker URL (map domain->url, forward all others)
    const worker = new URL(CF_URL);
    worker.searchParams.set("url", origin);
    for (const [k, v] of u.searchParams) if (k !== "domain") worker.searchParams.set(k, v);
    ...
    // -------- RAW: stream-through (no CPU-heavy buffering) --------
    if (raw) {
      const headers = new Headers(cors(h));
      headers.set("cache-control", "no-store");
      // Preserve upstream content-type when available
      headers.set("content-type", wres.headers.get("content-type") || "application/json");
      // Stream body to client; avoid .text()/.json() to keep CPU near-zero
      if (wres.body) {
        return new Response(wres.body, { status: wres.status, headers });
      } else {
        // Fallback (rare): upstream had no body; forward text minimally
        const text = await wres.text();
        return new Response(text, { status: wres.status, headers });
      }
    }

    // -------- Non-raw: parse JSON and persist to storage --------
    if (!wres.ok) {
      const text = await wres.text();
    ...
    const payload = (await wres.json()) as {
      sections?: Array<{
        id?: string; role?: string; bbox?: number[];
        image?: unknown; images?: unknown[];
        image_b64?: unknown; images_b64?: unknown[];
    ...
    if (!bucket) {
      return new Response(JSON.stringify({ error: "Storage binding CLOUD_FILES is missing" }), {
    ...
    const buildUrl = (key: string) => {
      const relative = `${basePath}/api/images/${key}`;
      return absolute ? `${siteOrigin}${relative}` : relative;
    };
    ...
    type Out = {
      ...
      image_b64?: string; images_b64?: string[];
      confidence?: number; label?: string;
    };

    const sections = await pMap<In, Out>(list as In[], UPLOAD_CONCURRENCY, async (s, idx) => {
      ...
      if (Array.isArray(s.images) && s.images.length) {
        ...
        if (includeB64) {
          if (Array.isArray(s.images_b64) && s.images_b64.length) {
            ...
            const b64s: string[] = [];
            for (const img of s.images) { const b64 = safeBase64FromDataUrl(img); if (b64) b64s.push(b64); }
      ...
      if (includeB64) {
        if (typeof s.image_b64 === "string") out.image_b64 = s.image_b64;
        if (Array.isArray(s.images_b64)) out.images_b64 = s.images_b64.filter((x): x is string => typeof x === "string");
      }
      return out;
    });
